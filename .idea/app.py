...
from online_bank import create_user, get_user, get_user_credit_card, create_save_graphs, email_verification, \
    delete_user_story_card, update_password, update_user, add_card, card_transfer, wire_transfer
from datetime import datetime
from email_management import send_email

app = Flask(__name__)
app.secret_key = '26f39dc632b5b24cbdca563ee5969af42e67b789'
...
@app.route("/Custom")
def custom():
    return render_template('customMain.html', user_name=session['user_nickname'])
...
def update_profile():
    ...
    return render_template('update_profile.html', user_nickname=session['user_nickname'],
                           user_name=session['user_name'], user_surname=session['user_surname'],
                           user_age=session['user_age'], user_birthday=session['user_birthday'],
                           user_email=session['user_email'], user_number=session['user_number'],
                           user_gender=session['user_gender'], user_address=session['user_address'])
    ...
    if request.method == "POST":
        if 'add_btn' in request.form:
            ...
            add_card(id_user, request.form['name'], request.form['surname'], request.form['number_card'],
                     request.form['cvv2'], expiration)
        elif 'radio_card' in request.form:
            if 'send_btn' in request.form:
                card_transfer(request.form['radio_card'], request.form['send_int'], id_user)
            elif 'get_btn' in request.form:
                wire_transfer(request.form['radio_card'], request.form['get_int'], id_user)
        if id_user is not None:
            c_list = get_user_credit_card(id_user)
            return render_template('customWallet.html', user_nickname=session['user_nickname'],
                                   user_money=session['user_money'],
                                   c_list=c_list, error=error)
...
def registration():
    error = None
    if request.method == "POST":
        if request.form['password1'] != request.form['password2'] and len(request['password1']) >= 8:
